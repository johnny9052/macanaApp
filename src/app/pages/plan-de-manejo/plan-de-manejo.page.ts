...
@Component({
  selector: 'app-plan-de-manejo',
  templateUrl: './plan-de-manejo.page.html',
  styleUrls: ['./plan-de-manejo.page.scss'],
})
export class PlanDeManejoPage implements OnInit {
  /* Se obtiene la url global definida en el archivo environment*/
  baseUrl = environment.baseUrl;

  /*Se define una lista de objetos de tipo PlandeManejo*/
  plandemanejo: ModelPlanDeManejo[] = [];
  ...
  constructor(
    ...
    private plandemanejoService: PlanDeManejoService,
  ...
  getPlanDeManejoData() {
    ...
    this.plandemanejoService.getPlanDeManejo().subscribe(data => {
      let res: any;
      res = data;
      /*Se convierte en un objeto JSON el listado de datos obtenido*/
      this.plandemanejo = JSON.parse(res.data);
  ...
  viewPlanDeManejo(id: string, fecha: string,nombre: string,idResponsable: string,observaciones: string) {
    ...
    variable lo llamamos data*/
     ...
     this.router.navigate(['plan-de-manejo-detail'], data);
  }
}
